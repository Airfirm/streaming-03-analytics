...
Terminal commands to run from the root project folder:
  ...
  Delete the topic and recreate it (start fresh):
    uv run python -m streaming.kafka_admin_femi --recreate

  Delete the topic only:
    uv run python -m streaming.kafka_admin_femi --delete
...
def main() -> None:
    ...
    if topic_exists(admin, settings.topic):
        ...
        LOG.info(
            "To start fresh, run:  uv run python -m streaming.kafka_admin_femi --recreate"
        )
    else:
